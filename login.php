...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // 以帳號進資料庫查詢
    $sql = "SELECT `username`, `name`, `role`, `password`, `email` FROM `user` WHERE `username`=?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "s", $username);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $result_username, $result_name, $result_role, $result_password, $result_email);
...
<body class="text-center">
    <main class="form-signin w-100 m-auto">
        <form method="POST" action="">
            ...
            <h1 class="h1 mb-4 fw-bold">登入Login</h1>
